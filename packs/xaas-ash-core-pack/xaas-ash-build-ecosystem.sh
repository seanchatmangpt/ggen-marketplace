...
# ash_onetime's own README documents `mix ash_onetime.install` as the canonical
# installer. The speedrun proved that routing this package through
# `mix igniter.install ash_onetime` did not resolve its installer task in the
# target project. Add/fetch the dependency first, then invoke the package task
# exactly as documented by the package.
