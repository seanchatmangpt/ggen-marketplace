...
def test_r46_has_exactly_fifty_new_sensors():
    sensors = [ROOT / 'queries' / f'{n}_' for n in range(186,236)]
...
def test_replication_compiler_is_deterministic_and_non_actuating():
...
def test_live_fixture_binds_three_exact_heads():
    fixture = (ROOT/'fixtures'/'r46-live-replication.ttl').read_text()
    for sha in ['22743d39d31a19490d6c9db881d02f3690b24913','f4e1bce1efdcdc4f6c2531be9f66070950f7ec93','791758248859dbf4201952294825ab324531ee0f']:
        assert sha in fixture
    assert 'actuationPerformed true' not in fixture

def test_sensor_queries_preserve_select_only_authority():
    for p in (ROOT/'queries').glob('*.rq'):
        if p.name[:3].isdigit() and 186 <= int(p.name[:3]) <= 235:
            text = p.read_text().upper()
            assert 'SELECT' in text
            assert not re.search(r'\b(INSERT|DELETE|LOAD|CLEAR|DROP|CREATE|MOVE|COPY|ADD)\b', text)
