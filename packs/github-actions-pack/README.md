...
Every rendered workflow/action file opens with a two-line
`# GENERATED by ggen (github-actions-pack/templates/<template>). Do not edit.`
YAML comment (since v0.2.4). It is inert to GitHub Actions and to any YAML
parser; it exists so a consumer whose provenance discipline is per-file and
marker-based (beam4pm's `scripts/gate_m2_check.sh`, which discovers
manufactured files by that header, deletes them, regenerates, and diffs) sees
these files as manufactured output rather than silently skipping them.
