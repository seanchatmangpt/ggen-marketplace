...
The court machinery (packs/semantic-gate-witness-court-pack, qualified
repo-wide by scripts/check_gate_witness_courts.py) delegates semantic
execution to a caller-supplied runner invoked without a shell, with the
zero exit status meaning the specified expectation was observed. This file
is that runner instance for this pack -- the first concrete instance in the
marketplace; no prior implementation existed to reuse (recorded failed edge,
2026-09-25).
...
Semantics:
  pass  -- witness graph must SHACL-conform against ontology/shapes.ttl
           (with ontology.ttl as the ontology graph) AND return zero rows
           from every gate in gates/.
