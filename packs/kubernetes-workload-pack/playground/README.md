...
were re-run against the real `ggen` binary when v0.1.0 was current. The
`privileged-container` verdict changed in v0.2.0 and has been evaluated
through gate 020 on three SPARQL engines, not yet the `ggen` binary.
...
The `mutable-image` known-gap row and the (now refused) `privileged-container` row are the same two files listed in
...
ResourceQuota, HorizontalPodAutoscaler, and a typed (non-raw-block) capabilities
model (the `privileged`/`allowPrivilegeEscalation` half is gated by
gate 020 since v0.2.0; `capabilities.add` is not) -- all still
hand-authored YAML in xaas's own `k8s/` today, or (for the typed
capabilities `add` list) not built anywhere yet. Each is a real candidate for
