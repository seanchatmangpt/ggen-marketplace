        ...
        if query_rel:
            ...
            if not query.is_file():
                ...
                if "SELECT" in text.upper() and "ORDER BY" not in text.upper():
                    failures.append(f"{pack_dir.name}:{name}:SELECT query lacks ORDER BY: {query_rel}")
        ...
        if template_rel:
            ...
            if not template.is_file():
                ...
                if "for row in rows" in text:
                    failures.append(f"{pack_dir.name}:{name}:legacy rows context in {template_rel}; use admitted results")
    return failures
